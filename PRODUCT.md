...
- Hospital selection ยังเป็น local screen context ของงาน Patient Provisioning, Patient Activation และ Hospital Patient Directory ไม่ใช่ global application state; OSM assigned-Patient directory ใช้ assignment scope ที่ server resolve แยกต่างหาก
...
- Phase 8B.0 Goals & Activity Plan working prototype implement แล้วภายใต้ Patient detail สำหรับดู history, สร้าง Goal Plan รอบใหม่โดยต้องเลือก Primary Goal เอง, configure weekly activities, review และดู immutable detail โดยใช้ Screening ได้เฉพาะ context/default ที่ผู้ใช้ review และ submit เอง
- Phase 9B.0 Appointment working prototype implement แล้วภายใต้ Patient relationship สำหรับดู bounded history, สร้าง, reschedule, cancel, complete และ mark no-show โดยใช้ `PatientAppointment`, server-side policy, atomic audit และ retry/stale-update protection; Appointment types, statuses, responsible-person rules และ actor authority ยังเป็น provisional requirement-validation behavior และ Phase 9C.0 Follow-up ยังไม่ implement
...
- Phase 9B.0 ใช้ provisional capability vocabulary `appointment:read` และ `appointment:manage`; active direct Hospital OWNER/MEMBER ได้ทั้งสอง capability, active exact-assignment OSM ได้เฉพาะ read, ส่วน OSM ที่ไม่มี assignment, PATIENT และ Platform ADMIN ถูก deny; profession และ Hospital hierarchy ไม่เปลี่ยน authority และ policy นี้ยังรอ customer confirmation
- Phase 8B.0 เก็บ Goal Plan เป็น immutable rounds (`PatientGoalPlan` + `PatientGoalItem`) พร้อม source template key/version, optional Screening reference ที่เก็บอัตโนมัติเมื่อ latest Screening ถูกใช้สร้าง prototype defaults, bounded retry nonce และ atomic `goal_plan.created` audit; Goal กับ Screening คง capability แยกกัน โดย Goal history เก็บเฉพาะ opaque Screening source ID และขอ minimal historical summary ผ่าน Screening-owned `screening:read` batch boundary; หากอ่าน Screening ไม่ได้ยังอ่าน Goal history/detail ที่ได้รับอนุญาตได้ และ history แสดงล่าสุดไม่เกิน 50 รอบ; ไม่มี edit/delete/correction/adherence/progress หรือ clinical recommendation
