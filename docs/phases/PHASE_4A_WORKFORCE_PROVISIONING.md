...
- วันที่: 2026-08-14
- ขอบเขต: Hospital personnel และ OSM provisioning เท่านั้น
...
5. การต่อ ActorContext ให้รู้จัก OSM–Hospital relationship ขั้นต่ำ หากใช้ persistence model ใหม่
...
ตอนนี้ยังไม่มีวิธี activation/credential-ownership ของ staff/OSM ที่ยืนยันแล้ว ดังนั้น Phase 4B มีสองระดับผลลัพธ์:

| ระดับ | ผลลัพธ์ที่อนุญาต | สถานะ |
| --- | --- | --- |
| Provisioning-complete | Person/User, trusted role และ Hospital relationship อยู่ในสถานะ `PROVISIONED`; ยังเข้า `/app` ไม่ได้ | Implement ได้โดยไม่ invent activation |
| Activation-complete | credential ownership พิสูจน์แล้ว, `User` และ relationship ที่เกี่ยวข้องเป็น `ACTIVE`, login ผ่าน boundary เดิมได้ | ทำได้ต่อเมื่อ owner ยืนยัน activation contract ในข้อ 14 |

ห้ามรายงาน Phase 4B ว่าเป็น end-to-end `ACTIVE` slice หากยังไม่มีวิธีพิสูจน์ credential ownership ที่อนุมัติแล้ว ห้ามแก้ปัญหานี้ด้วย password ที่ Hospital Owner สร้าง/รู้ หรือด้วยการ mark `ACTIVE` ล่วงหน้า
...
| Authorized Hospital actor | เป็นความเป็นไปได้ในอนาคต แต่ยังไม่มี capability mapping ที่ยืนยันแล้ว; safe default ของ Phase 4B คือไม่อนุญาต ordinary `HOSPITAL` member ให้ provision |
| Target Hospital personnel | ผู้รับ `HOSPITAL` role และ `HospitalMembership` แบบ `MEMBER`; ไม่มี authority จากข้อมูลที่ส่งเข้ามา |
| Target OSM | ผู้รับ `OSM` role และ minimum Hospital relationship; ไม่ได้รับ area/patient/clinical scope จากการ provision นี้ |
...
Person/User + trusted role + Hospital relationship
      ↓ target-owned activation, when confirmed
ACTIVE ActorContext
```
...
สำหรับ existing `User` ที่มี `status = ACTIVE`, มี `authSubject` ที่ map ถูกต้อง และมี account credential ownership อยู่แล้ว safe default คือสร้าง relationship ใหม่เป็น `ACTIVE` ใน transaction เดียว โดยไม่เรียก provider และไม่เปลี่ยน credential เดิม การต้องให้ target ยืนยัน/accept relationship ใหม่นี้เป็น open requirement ที่อาจเปลี่ยน default ดังกล่าว
...
Phase 4B ต้องเก็บเพียงความสัมพันธ์ขั้นต่ำว่า OSM ถูก provision/associated โดย Hospital ใด และสถานะของความสัมพันธ์เป็นอะไร ความสัมพันธ์นี้เป็น tenant association ไม่ใช่การตัดสิน clinical/resource scope

โมเดลแนะนำและเหตุผลอยู่ในข้อ 8:
...
ยังไม่ยืนยันว่า OSM scope เป็น Hospital, area, assigned patients, geographic hierarchy หรือการผสมกัน ดังนั้น:
...
โครงสร้าง database อาจรองรับ OSM relationship หลาย Hospital เพื่อไม่ทำลาย identity reuse แต่ product behavior ว่า OSM หนึ่งคน “ควร” อยู่ได้หลาย Hospital หรือไม่ยังไม่ถูกเปิดเป็น feature contract
...
create HospitalMembership(MEMBER, profession, PROVISIONED)
  ↓
activation remains separate until confirmed
```
...
- ถ้า User เดิม `ACTIVE` และ auth mapping ถูกต้อง safe default คือ membership ใหม่ `ACTIVE`; ถ้า User ยังไม่ active ให้ relationship ใหม่เป็น `PROVISIONED`
...
### 6.3 Open activation requirement

ยังไม่ได้ยืนยันว่าจะใช้ email invitation, phone OTP, activation token, external identity, ThaID หรือวิธีอื่น และยังไม่มี expiry, revocation, recovery, existing-user acceptance หรือ target-channel ownership contract สำหรับ staff/OSM

จนกว่าจะยืนยัน:

- Hospital Owner ห้ามรับหรือเห็น password, provider alias, provider subject, token หรือ secret
- ห้าม derive password จาก National ID, วันเกิด หรือข้อมูล profile
- ห้าม persist plaintext credential
- ใหม่/ยังไม่ active ใช้ `User.PROVISIONED` และ relationship `PROVISIONED`; ไม่ใช้ `INVITED` หากยังไม่มี invitation ที่ถูกสร้างจริง
- `User.SUSPENDED` ห้ามถูก activate อัตโนมัติ
- provider call ไม่อยู่ใน core local provisioning transaction; หาก future activation ต้องเรียก provider ให้ reuse compensation/reconciliation boundary เดิม

## 7. Authorization and Scope

### 7.1 Minimum Phase 4 capability vocabulary

ชื่อด้านล่างเป็น **ข้อเสนอ vocabulary ระดับ phase** ที่จัดรูปตาม convention `resource:action`; ยังไม่ใช่ final global capability matrix:

| Candidate capability | ใช้กับ | เงื่อนไขขั้นต่ำ |
| --- | --- | --- |
...
- ordinary `HOSPITAL` member ยังไม่มีสิทธิ์ provision ใน safe default; การเปิด capability นี้ต้องมี owner confirmation
...
ดังนั้น Phase 4B **แนะนำ** model ใหม่เชิงแนวคิดชื่อ `OsmHospitalRelationship` หรือชื่อที่ตรงกับ repository convention โดยมีขั้นต่ำ:
...
- Phase 4B ไม่ expose behavior ว่า OSM สามารถใช้งานหลาย Hospital พร้อมกันจนกว่า owner จะยืนยัน product semantics

การสร้าง model นี้เป็น schema change ที่ต้องผ่าน migration ใน Phase 4B ไม่ใช่การแก้ schema ใน Phase 4A หาก owner เลือกใช้ `HospitalMembership` แทน ต้องปิดคำถามนี้ก่อน implement เพราะจะเปลี่ยน relation invariant, ActorContext และ policy query

### 8.3 ActorContext boundary
...
หาก owner ยืนยัน activation mechanism ในภายหลัง ให้แยกเป็น operation ที่มี boundary ของตัวเอง:

1. target เป็นผู้พิสูจน์ credential ownership ผ่าน channel ที่ยืนยันแล้ว
2. provider/Auth effect ทำผ่าน existing server-only adapter นอก PostgreSQL transaction
3. ตรวจ `User.authSubject` mapping และ expected User/relationship state
4. transaction ใหม่เปลี่ยน `User.PROVISIONED` หรือ state ที่อนุญาตเป็น `ACTIVE`, เปลี่ยน relationship ที่เกี่ยวข้องเป็น `ACTIVE` และเขียน activation audit
5. ถ้า provider สำเร็จแต่ final PostgreSQL write ล้มเหลว ให้ใช้ compensation/reconciliation ของ Phase 2.1; ห้ามสร้าง success จาก partial state

การ provision ที่จบใน `PROVISIONED` เป็น valid non-authoritative state เมื่อยังไม่มี activation proof ไม่ใช่ partial active authority และต้องไม่เข้า `/app`
...
Audit event names ด้านล่างเป็น semantic contract ที่เสนอให้ใช้ตาม convention ปัจจุบัน (`entity.operation`) และควรรวมเป็น constants/validation กลางใน Phase 4B หาก repository ตั้งชื่อที่ต่างแต่ความหมายเท่ากัน ให้เปลี่ยนชื่อในจุดเดียวโดยไม่เปลี่ยน invariant
...
| `workforce.activation_completed` หรือ event แยกตาม relationship | `User`/relationship | target/session/system ตาม activation contract ที่อนุมัติ | credential ownership proof และ ACTIVE transition สำเร็จ |
| `membership.suspended` | relationship | authorized actor ที่ได้รับอนุมัติ | ไม่อยู่ใน Phase 4B จนกว่าจะมี suspend requirement |

Provisioning event ต้องถูกเขียนใน transaction เดียวกับ role/relationship writes และไม่เขียน event ซ้ำเมื่อ retry เป็น idempotent no-op การ activation event ต้องไม่เขียนก่อน final active state commit
...
- [ ] Confirm owner decisions ในข้อ 14 ก่อนเปิด full ACTIVE acceptance
- [ ] ใช้ `Person`, `User`, `UserRole`, `Hospital`, `HospitalMembership` และ `AuditEvent` เดิมสำหรับ staff
- [ ] เพิ่ม migration สำหรับ OSM relationship เฉพาะเมื่อ model แยกได้รับการยืนยัน; ห้ามเพิ่ม area/patient/geographic fields
...
- [ ] ใช้ `membership:read`, `membership:create`, `osm:provision` เฉพาะตาม semantics ที่ owner ยืนยันและรวม policy ไว้จุดเดียว
...
- [ ] เมื่อ activation method ยังไม่ยืนยัน ให้จบที่ `PROVISIONED` และไม่เรียก password provider เพื่อสร้าง credential แทน target

### ActorContext and login boundary

- [ ] เพิ่ม OSM relationship ขั้นต่ำใน server-resolved ActorContext หากมี model ใหม่ โดยไม่เพิ่ม patient/area scope
...
- [ ] provider subject already exists, provider conflict, partial provider failure และ reconciliation path เมื่อ activation ถูกเพิ่ม
...
- [ ] `/login`/ActorContext acceptance ของ ACTIVE staff/OSM ต้องรอ activation decision; ห้าม test ผ่านด้วยการ mark active แบบ hardcoded
...
## 14. Open Requirements Requiring Owner Confirmation

ตารางนี้เก็บเฉพาะคำถามที่ block full Phase 4B หรือเปลี่ยน invariant/policy อย่างมีนัยสำคัญ:

| Question | Why it matters | Safe default / ต้องรอหรือไม่ |
| --- | --- | --- |
| Staff/OSM จะ establish credential ownership และ activate ผ่าน channel ใด | กำหนดว่าใครเป็นเจ้าของ secret, provider flow, expiry, revocation, recovery และเมื่อใดจึง `ACTIVE` | Provisioning-only ทำได้ถึง `PROVISIONED`; ต้องรอก่อน implement full ACTIVE/activation |
| Existing `User.ACTIVE` ที่ถูกเพิ่ม Hospital role/relationship ต้องยืนยันซ้ำหรือไม่ | เปลี่ยนจากการ activate relationship ทันทีเป็น invitation/consent lifecycle และอาจต้องมี state/token | Safe default คือ relationship `ACTIVE` เมื่อ auth mapping ถูกต้องและ owner เป็นผู้ provision; รอถ้าต้องมี target acceptance |
| Ordinary `HOSPITAL` member ที่ไม่ใช่ OWNER มีสิทธิ์ provision ได้หรือไม่ | เปลี่ยน policy actor, capability assignment และ risk boundary | Safe default: OWNER เท่านั้น; implementation ไม่ต้องรอถ้ายอมรับ default นี้ |
| OSM ใช้ model แยก `OsmHospitalRelationship` หรือ reuse `HospitalMembership` | เปลี่ยน schema migration, query, ActorContext และ invariant ของ OSM association | แนะนำ model แยกตามข้อ 8; ต้องยืนยันก่อน migration/Phase 4B persistence code |
| OSM หนึ่งคน belong/operate ได้หลาย Hospital หรือไม่ | เปลี่ยน uniqueness/policy/workspace semantics และอาจกระทบ scope | โครงสร้างรองรับหลาย row ได้เพื่อรักษา identity reuse แต่ไม่เปิด behavior ข้าม Hospital จนกว่าจะยืนยัน |
| OSM scope เป็น Hospital, area, assigned patients หรือ combination ใด | เปลี่ยน authorization และ future clinical data model | ห้ามเพิ่ม scope ใน Phase 4B; ไม่ block minimum association แต่ block scope-enabled workflows |
| Hospital Owner สร้าง OWNER เพิ่ม, มีหลาย OWNER หรือ transfer ownership ได้หรือไม่ | เปลี่ยน membership invariant และ governance/audit | Deny/defer ทั้งหมดใน Phase 4B; ไม่ต้องรอถ้า MVP ใช้ existing Owner เท่านั้น |
| Parent/main Hospital มีสิทธิ์จัดการ child Hospital หรือไม่ | `parentHospitalId` อาจกลายเป็น authorization scope แทน metadata | Safe default: ไม่มี inherited authority; ไม่ต้องรอสำหรับ direct Hospital-only MVP |
| Staff สามารถย้าย/ผูกหลาย Hospitalผ่าน workforce operation หรือไม่ | เปลี่ยนจาก add relationship เป็น transfer/update และต้องกำหนด audit/status | Safe default: เพิ่ม relationship ได้ตาม direct target แต่ไม่มี move/downgrade/transfer operation |
| OSM สามารถมีทั้ง Hospital personnel membership และ OSM relationship ใน Hospital เดียวกันหรือไม่ | เปลี่ยน conflict rules ของ role combination และ UI/ActorContext | Identity model รองรับ multi-role; Phase 4B ไม่ลบ relationship เดิม และใช้ separate operation; รอเฉพาะถ้าธุรกิจห้าม combination |
| Required staff/OSM profile fields นอกจาก National ID, given/family name, target Hospital และ profession มีอะไรบ้าง | อาจต้องเพิ่ม Person/profile schema, validation และ PII handling; legacy มี birth date/contact/specialization แต่ current schema ยังไม่มี | Safe default ใช้ minimum fields ในข้อ 3.2; รอเฉพาะถ้าฟิลด์เหล่านั้นเป็น acceptance criterion ของ MVP |

คำถามที่เป็น owner-management, parent/child authority, staff transfer และ OSM clinical scope ไม่ควรถูกปิดด้วย legacy behavior หรือ implementation convenience

## 15. Legacy Behavior Review

### 15.1 Business evidence ที่นำมาใช้เป็น requirement evidence

จาก [legacy staff add flow](https://github.com/raviut-max/demi-plus-web-v2/blob/7a5510ee1cb5c55b62ad62b0d49bbaa8295d228e/app/admin/staff/add/page.tsx), [legacy staff registration flow](https://github.com/raviut-max/demi-plus-web-v2/blob/7a5510ee1cb5c55b62ad62b0d49bbaa8295d228e/app/admin/staff/register/page.tsx), [legacy staff management](https://github.com/raviut-max/demi-plus-web-v2/blob/7a5510ee1cb5c55ad62b0d49bbaa8295d228e/app/admin/staff/page.tsx) และ [legacy Supabase queries](https://github.com/raviut-max/demi-plus-web-v2/blob/7a5510ee1cb5c55ad62b0d49bbaa8295d228e/lib/supabase/queries.ts) พบพฤติกรรม/คำศัพท์ที่เป็น evidence ได้:
...
- Scope ตรงกับ Phase 4A requirement closure; ไม่มี UI, Server Action, route, migration, activation, OTP, provider integration หรือ product code ถูกเพิ่ม
- Handoff ระบุ activation, OSM model, OSM scope, actor policy และ governance questions ที่ต้องยืนยันก่อน/ระหว่าง Phase 4B
...
- Markdown links ใช้ relative repository paths หรือ fixed legacy commit links; ไฟล์ใหม่ต้องเป็น UTF-8 without BOM

### Documentation synchronization decision

Phase 4A ไม่แก้ `README.md`, `PRODUCT.md`, `docs/CONTEXT.md` หรือ `docs/architecture/DEMI_ARCHITECTURE_BASELINE.md` เพราะยังไม่มี decision ใหม่ที่ยืนยันจนควรเปลี่ยน current project summary การเปลี่ยน capability, OSM relationship model, activation mechanism หรือ lifecycle เป็น accepted cross-module architecture decision ควรทำผ่าน ADR ใหม่และ sync baseline/context ใน change เดียวกันก่อน implement migration/feature code

ไม่มี ADR ใหม่ถูกสร้างใน Phase 4A เพื่อไม่ promote ข้อเสนอ OSM model หรือ activation mechanism ให้กลายเป็น accepted architecture โดยไม่มี owner confirmation
