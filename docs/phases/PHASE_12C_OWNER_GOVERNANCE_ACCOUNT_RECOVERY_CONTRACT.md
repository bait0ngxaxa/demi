...
| Hospital membership is suspended | Relationship lifecycle | Exact Hospital relationship is `SUSPENDED`; Phase 11B.0 supports bounded `MEMBER` membership suspend/restore under exact Owner authority, while `OWNER` rows are excluded | Use membership lifecycle policy. It is not account recovery. |
