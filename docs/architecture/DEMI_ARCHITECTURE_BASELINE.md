...
- A focused integration suite verifies the PostgreSQL constraints against a dedicated test database. Local development uses the disposable PostgreSQL service in `compose.integration.yaml`, configured by `.env.integration` and bound only to `127.0.0.1:55432`. A Docker-enabled WSL terminal starts the container and leaves it running; `npm run test:integration` then validates the local environment, runs `prisma generate`, applies migrations with `prisma migrate deploy`, and runs the integration tests without invoking Docker or WSL. `npm run test:integration:local` remains an optional full-lifecycle wrapper. The commands are documented in the repository [README](../../README.md). Integration commands reject `NODE_ENV=production`, require `DATABASE_URL`, `DIRECT_URL`, and `DEMI_TEST_DATABASE_URL` to identify the same test database, and validate that the database host is local.
