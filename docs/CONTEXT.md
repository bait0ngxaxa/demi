...
**Phase 14B.2 Loading UI System & Route Coverage ปิดแล้ว** ตาม [Phase 14B.2 handoff](./phases/PHASE_14B2_LOADING_UI_SYSTEM_ROUTE_COVERAGE.md) โดยกำหนด pattern กลางว่า route/data loading ใช้ structural skeleton + shimmer (พร้อม static reduced-motion fallback) ส่วน mutation/action loading คง button spinner + pending label, เพิ่ม loading boundaries ตาม route family ทั้ง protected/public และเพิ่ม desktop sidebar แบบ expanded/collapsed ที่เก็บ UI preference ฝั่ง browser โดยไม่เปลี่ยน navigation authorization, mobile drawer, business หรือ security behavior
