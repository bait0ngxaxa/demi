...
- The prototype uses source-defined `demi-goals` / `legacy-prototype-v1` definitions, immutable `PatientGoalPlan` rounds with `PatientGoalItem` snapshots, Screening context through the Screening-owned `screening:read` boundary, automatic retention of the latest Screening source when it supplies prototype defaults, server-side template validation, serializable round allocation, per-form retry nonce, and bounded `goal_plan.created` audit.
