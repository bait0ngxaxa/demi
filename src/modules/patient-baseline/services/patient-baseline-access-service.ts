...
import {
  assertPatientBaselinePolicy,
  type PatientBaselineCapability,
  type PatientBaselinePolicyTarget,
} from "../policies/patient-baseline-policy";
import { patientBaselineRelationshipIdSchema } from "../schemas/patient-baseline-schemas";

export type PatientBaselineAccessDatabase = PrismaClient | Prisma.TransactionClient;

export const patientBaselineRelationshipAccessSelect = {
  id: true,
  hospitalId: true,
  hospitalNumber: true,
  hospital: {
...
export type PatientBaselineRelationshipAccessRecord = Prisma.PatientHospitalRelationshipGetPayload<{
  select: typeof patientBaselineRelationshipAccessSelect;
}>;

export type PatientBaselinePatientSummary = {
...
export type PatientBaselineAccessContext = {
  patient: PatientBaselinePatientSummary;
  target: PatientBaselinePolicyTarget;
  actor: ActorContext;
};

function getDatabase(database?: PatientBaselineAccessDatabase): PatientBaselineAccessDatabase {
...
function hasPatientRole(record: PatientBaselineRelationshipAccessRecord): boolean {
  return Boolean(
...
export async function resolvePatientBaselineAccessContext(
  actor: ActorContext | null | undefined,
  relationshipId: unknown,
  capability: PatientBaselineCapability,
  database?: PatientBaselineAccessDatabase,
): Promise<PatientBaselineAccessContext> {
  if (!actor) {
    throw new ForbiddenError();
  }

  const parsedRelationshipId = patientBaselineRelationshipIdSchema.safeParse(relationshipId);
  ...
  const target: PatientBaselinePolicyTarget = {
